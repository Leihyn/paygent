// Escrow functions for the v3 SIP-010 contract.
// Same flow as escrow.ts but uses a fungible token (mock-sbtc) instead of native STX.
...
export async function createEscrowV3(
  ...
  if (isOfflineDemo) {
    console.log("[Paygent] OFFLINE MODE -- simulating v3 escrow creation");
    return { txId: "offline-tx-create-v3-" + Date.now(), jobId: 1 };
  }
  ...
  const txOptions = {
    ...
    contractName: "agentpay-escrow-v3",
  ...
  console.log(`[Paygent] v3 Escrow created. TxID: ${broadcastResponse.txid}`);
...
export async function completeEscrowV3(
  jobId: number,
  ...
  if (isOfflineDemo) {
    console.log("[Paygent] OFFLINE MODE -- simulating v3 escrow completion");
    return "offline-tx-complete-v3-" + Date.now();
  }
  ...
  if ("error" in broadcastResponse) {
    throw new Error(`Complete v3 escrow failed: ${broadcastResponse.error}`);
  }

  console.log(`[Paygent] v3 Escrow completed. TxID: ${broadcastResponse.txid}`);
  return broadcastResponse.txid;
}

// Read-only: fetch v3 escrow state from the contract.
export async function getEscrowV3(jobId: number): Promise<EscrowJob | null> {
  if (isOfflineDemo) {
    return {
      jobId,
...
async function pollForJobId(txId: string): Promise<number> {
  ...
  console.log(`[Paygent] Polling for v3 tx confirmation: ${txId}`);
    ...
    try {
      ...
      if (tx.tx_status === "success") {
        ...
        if (match) {
          ...
          console.log(`[Paygent] v3 Tx confirmed in ${attempt * 5}s. Job ID: ${jobId}`);
          return jobId;
        }
        // Fallback: read job counter if repr format is unexpected
        console.log("[Paygent] v3 Tx confirmed but couldn't parse job ID from result, reading job counter...");
        const result = await callReadOnlyFunction({
          contractAddress: config.contract.address,
          contractName: "agentpay-escrow-v3",
      ...
      console.log(`[Paygent] Waiting for v3 confirmation... (${attempt * 5}s)`);
    } catch (err: any) {
